...
class StemLayer(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv2 = nn.Conv2d(out_chans // 2,
                               ...
                               stride=2,
                               padding=1)
        # self.norm2 = build_norm_layer(out_chans, norm_layer, 'channels_last',
        #                               'channels_last')
...
class CustomVisionEncoderDecoderModel(VisionEncoderDecoderModel):
    def __init__(self, config):
        ...
        # Load encoder and decoder weights if paths are provided in config
        print('config', config)
        if hasattr(config, 'encoder_checkpoint_path') and config.encoder_checkpoint_path:
            print("load encoder checkpoint")
            self.load_encoder_checkpoint(config.encoder_checkpoint_path)
            print("load encoder checkpoint success")
        
        if hasattr(config, 'decoder_checkpoint_path') and config.decoder_checkpoint_path:
            print("load decoder checkpoint")
            self.load_decoder_checkpoint(config.decoder_checkpoint_path)
        ...
        # Freeze decoder if required
        if hasattr(config, 'freeze_decoder') and config.freeze_decoder:
            for param in self.decoder.parameters():
                param.requires_grad = False
            print("Decoder has been frozen.")
            
    def load_encoder_checkpoint(self, url_or_filename):
        """
        Load encoder weights from a checkpoint.
        """
        return self.load_checkpoint(url_or_filename)

    def load_decoder_checkpoint(self, url_or_filename):
        """
    ...
    def load_checkpoint(self, url_or_filename):
        ...
        print(f"Missing keys exist when loading encoder from '{url_or_filename}'.")
        print(f"load encoder checkpoint from {url_or_filename}")
