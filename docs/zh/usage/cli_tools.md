...
- `MINERU_FORMULA_CH_SUPPORT`：
    * 用于启用 PP-FormulaNet_plus-L 中文公式解析优化（实验性功能）。
    * 默认为`false`，此时使用默认的`unimernet_small`公式识别模型；设置为`true`后使用`PP-FormulaNet_plus-L_safetensors`。
    * 使用 Hugging Face 或 ModelScope 模型源时，会从 pipeline 模型仓库的`models/MFR/pp_formulanet_plus_l/`相对路径下载 plus-L 模型。
    * 使用`MINERU_MODEL_SOURCE=local`时，请将 plus-L 模型文件放在`<pipeline模型根目录>/models/MFR/pp_formulanet_plus_l/`下，例如该目录内包含`config.json`、`generation_config.json`、`model.safetensors`、`processor_config.json`、`tokenizer.json`等文件。
    * plus-L 默认使用`bf16`推理以降低显存占用；可通过`MINERU_FORMULA_PLUS_L_DTYPE=fp32`切回`fp32`，或设置为`auto`按设备能力自动选择。
    * 仅对`pipeline`后端生效。
